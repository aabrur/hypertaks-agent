...
- **Inputs:** Slovin: N, margin of error e. Power: 3 of `alpha`, power `1 - beta`, minimum effect size, `n`. **If e is not provided, it is `DATA UNAVAILABLE`. Do not assume e = 0.05 or any other default. If any required input is missing, the final result must be stated as `DATA UNAVAILABLE` and no numeric calculation may be performed.**
...
- **Output shape:** A computation block containing inputs, formulas, substitutions, result, assumptions, and claim ceiling. (If an input is `DATA UNAVAILABLE`, the substitutions and result are also `DATA UNAVAILABLE`).
- **Traps:**
