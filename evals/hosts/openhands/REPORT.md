# OpenHands Adapter Audit Report

- **Host ID**: `openhands`
- **Official Display Name**: OpenHands
- **Official Documentation**: https://github.com/All-Hands-AI/OpenHands
- **Tested Version**: 0.18.0 (from adapter metadata; not independently observed in this session)
...
1. **Manifest Presence**: the registered adapter path exists and is structurally valid.
   - Adapter path: `.openhands/plugin.json`
2. **Registry Entry**: `distribution/registry.json` maps `openhands` to distribution type `managed-install`.
...
- Invocation: agent-skill-and-event-stream
- Update mechanism: hypertaks-update
- Uninstall mechanism: hypertaks-uninstall
- Host tool mapping: openhands-action-space
- Subagent model: native-micro-agent
- Persistent memory: file-backed-and-memory-store
- Filesystem availability: docker-container-filesystem
- Command execution: docker-bash-terminal
- Required account/plan: open-source
- MCP requirement: optional
- Known restrictions: managed installation registers micro-agent skills in .openhands/

## Unverified Items (Require Manual Host Test)

1. **Installation**: run `hypertaks install openhands --scope project`; verify activation in the real host.
...
5. **Tool Mapping**: verify capability routing matches openhands-action-space.
...
8. **Update Mechanism**: `hypertaks update openhands` fast-forwards cleanly; rejects dirty/diverged/detached states.
9. **Uninstall Mechanism**: `hypertaks uninstall openhands` removes only Hypertaks files; `hypertaks verify openhands` then reports NOT_INSTALLED.
