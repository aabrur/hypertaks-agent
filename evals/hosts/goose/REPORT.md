# Goose Adapter Audit Report

- **Host ID**: `goose`
- **Official Display Name**: Goose
- **Official Documentation**: https://block.github.io/goose/docs
- **Tested Version**: 1.0.2 (from adapter metadata; not independently observed in this session)
...
1. **Manifest Presence**: the registered adapter path exists and is structurally valid.
   - Adapter path: `.goose/plugin.json`
2. **Registry Entry**: `distribution/registry.json` maps `goose` to distribution type `managed-install`.
...
- Invocation: recipe-and-extension-invocation
- Update mechanism: hypertaks-update
- Uninstall mechanism: hypertaks-uninstall
- Host tool mapping: goose-toolkit
- Subagent model: synthesized
- Persistent memory: file-backed
- Filesystem availability: full-read-write
- Command execution: full-terminal-execution
- Required account/plan: open-source
- MCP requirement: optional
- Known restrictions: managed installation provisions recipe and extension files under .goose/

## Unverified Items (Require Manual Host Test)

1. **Installation**: run `hypertaks install goose --scope project`; verify activation in the real host.
...
5. **Tool Mapping**: verify capability routing matches goose-toolkit.
...
8. **Update Mechanism**: `hypertaks update goose` fast-forwards cleanly; rejects dirty/diverged/detached states.
9. **Uninstall Mechanism**: `hypertaks uninstall goose` removes only Hypertaks files; `hypertaks verify goose` then reports NOT_INSTALLED.
