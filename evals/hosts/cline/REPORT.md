# Cline Adapter Audit Report

- **Host ID**: `cline`
- **Official Display Name**: Cline
- **Official Documentation**: https://github.com/cline/cline
- **Tested Version**: 3.0.47 (from adapter metadata; not independently observed in this session)
...
1. **Manifest Presence**: the registered adapter path exists and is structurally valid.
   - Adapter path: `.cline/plugin.json`
2. **Registry Entry**: `distribution/registry.json` maps `cline` to distribution type `managed-install`.
...
- Invocation: cline-custom-instructions-and-skills
- Update mechanism: hypertaks-update
- Uninstall mechanism: hypertaks-uninstall
- Host tool mapping: cline-native-tools
...
- Known restrictions: managed installation configures .cline/skills and custom prompt

## Unverified Items (Require Manual Host Test)

1. **Installation**: run `hypertaks install cline --scope project`; verify activation in the real host.
...
5. **Tool Mapping**: verify capability routing matches cline-native-tools.
...
8. **Update Mechanism**: `hypertaks update cline` fast-forwards cleanly; rejects dirty/diverged/detached states.
9. **Uninstall Mechanism**: `hypertaks uninstall cline` removes only Hypertaks files; `hypertaks verify cline` then reports NOT_INSTALLED.
