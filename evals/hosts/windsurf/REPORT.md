# Windsurf Adapter Audit Report

- **Host ID**: `windsurf`
- **Official Display Name**: Windsurf
- **Official Documentation**: https://codeium.com/windsurf/docs
- **Tested Version**: 1.9.1 (from adapter metadata; not independently observed in this session)
...
1. **Manifest Presence**: the registered adapter path exists and is structurally valid.
   - Adapter path: `.windsurf/plugin.json`
2. **Registry Entry**: `distribution/registry.json` maps `windsurf` to distribution type `managed-install`.
...
- Invocation: cascade-command-and-natural-language
- Update mechanism: hypertaks-update
- Uninstall mechanism: hypertaks-uninstall
- Host tool mapping: windsurf-cascade-tools
...
- Required account/plan: codeium-account
- MCP requirement: optional
- Known restrictions: managed installation writes rules and skills to .windsurf/

## Unverified Items (Require Manual Host Test)

1. **Installation**: run `hypertaks install windsurf --scope project`; verify activation in the real host.
...
5. **Tool Mapping**: verify capability routing matches windsurf-cascade-tools.
...
8. **Update Mechanism**: `hypertaks update windsurf` fast-forwards cleanly; rejects dirty/diverged/detached states.
9. **Uninstall Mechanism**: `hypertaks uninstall windsurf` removes only Hypertaks files; `hypertaks verify windsurf` then reports NOT_INSTALLED.
