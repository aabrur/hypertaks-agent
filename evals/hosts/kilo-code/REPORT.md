# Kilo Code Adapter Audit Report

- **Host ID**: `kilo-code`
- **Official Display Name**: Kilo Code
- **Official Documentation**: https://kilo.ai/docs
- **Tested Version**: 1.1.0 (from adapter metadata; not independently observed in this session)
...
1. **Manifest Presence**: the registered adapter path exists and is structurally valid.
   - Adapter path: `.kilo/plugin.json`
2. **Registry Entry**: `distribution/registry.json` maps `kilo-code` to distribution type `managed-install`.
...
- Required account/plan: kilo-account
- MCP requirement: optional
- Known restrictions: managed skill adapter links canonical skills to .kilo/skills

## Unverified Items (Require Manual Host Test)

1. **Installation**: run `hypertaks install kilo-code --scope project`; verify activation in the real host.
...
5. **Tool Mapping**: verify capability routing matches kilo-tools.
...
8. **Update Mechanism**: `hypertaks update kilo-code` fast-forwards cleanly; rejects dirty/diverged/detached states.
9. **Uninstall Mechanism**: `hypertaks uninstall kilo-code` removes only Hypertaks files; `hypertaks verify kilo-code` then reports NOT_INSTALLED.
