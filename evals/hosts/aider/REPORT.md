# Aider Adapter Audit Report

- **Host ID**: `aider`
- **Official Display Name**: Aider
- **Official Documentation**: https://aider.chat/docs
- **Tested Version**: 0.72.0 (from adapter metadata; not independently observed in this session)
...
1. **Manifest Presence**: the registered adapter path exists and is structurally valid.
   - Adapter path: `.aider/plugin.json`
2. **Registry Entry**: `distribution/registry.json` maps `aider` to distribution type `project-instructions`.
...
- Invocation: read-prompt-and-conventions
- Update mechanism: hypertaks-update
- Uninstall mechanism: hypertaks-uninstall
- Host tool mapping: aider-git-and-file-tools
...
- Known restrictions: uses project instructions via .aider.conf.yml and CONVENTIONS.md

## Unverified Items (Require Manual Host Test)

1. **Installation**: run `hypertaks install aider --scope project`; verify activation in the real host.
...
5. **Tool Mapping**: verify capability routing matches aider-git-and-file-tools.
...
8. **Update Mechanism**: `hypertaks update aider` fast-forwards cleanly; rejects dirty/diverged/detached states.
9. **Uninstall Mechanism**: `hypertaks uninstall aider` removes only Hypertaks files; `hypertaks verify aider` then reports NOT_INSTALLED.
