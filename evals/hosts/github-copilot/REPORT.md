# GitHub Copilot Adapter Audit Report

- **Host ID**: `github-copilot`
- **Official Display Name**: GitHub Copilot
- **Official Documentation**: https://docs.github.com/en/copilot
- **Tested Version**: 1.0.70 (from adapter metadata; not independently observed in this session)
...
1. **Manifest Presence**: the registered adapter path exists and is structurally valid.
   - Adapter path: `.github-copilot/plugin.json`
2. **Registry Entry**: `distribution/registry.json` maps `github-copilot` to distribution type `native-plugin`.
3. **Skill Root**: manifest references `./skills/`; canonical skill directory present.
4. **Classification**: NATIVE_PLUGIN
...
- Invocation: slash-command-and-agent-mention
- Update mechanism: hypertaks-update
- Uninstall mechanism: hypertaks-uninstall
- Host tool mapping: copilot-workspace-tools
- Subagent model: synthesized
- Persistent memory: file-backed
- Filesystem availability: full-read-write
- Command execution: workspace-terminal
- Required account/plan: github-copilot-subscription
- MCP requirement: optional
- Known restrictions: requires .github-copilot/ or .github/ plugin specification

## Unverified Items (Require Manual Host Test)

1. **Installation**: run `hypertaks install github-copilot --scope project`; verify activation in the real host.
...
5. **Tool Mapping**: verify capability routing matches copilot-workspace-tools.
...
8. **Update Mechanism**: `hypertaks update github-copilot` fast-forwards cleanly; rejects dirty/diverged/detached states.
9. **Uninstall Mechanism**: `hypertaks uninstall github-copilot` removes only Hypertaks files; `hypertaks verify github-copilot` then reports NOT_INSTALLED.
